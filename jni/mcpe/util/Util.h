...
public:
	...
	{
		return Util::toUpperCase(current.substr(0, 1)) + current.substr(1);
	}
	...
	static std::string toUpperCase(const std::string& in)
	{
		std::string result;
		std::transform(in.begin(), in.end(), result.begin(), ::toupper);
	}
